...
- 工作区是可重建镜像，`.git` 必须保留。
- 自动本地同步快照、旧 `.diff-ref` 和完整工作区是三种格式；写入只使用同步快照，读取必须保持旧格式兼容。
- 同步对象以 SHA-256 命名且不可变；对象先写临时文件再原子发布，快照清单也必须在 staging 完整生成后发布。
- 元数据缓存只有在 `size + mtime + ctime + dev/ino` 全部一致时复用旧哈希；修改缓存判定时必须保留“任一项变化就重新哈希”的失败关闭语义。
- 清单路径必须使用安全的 POSIX 相对路径；恢复前校验块大小、块哈希和整文件哈希。
- 删除以完整清单中的缺失状态和 `deleted` 回收站记录表达，不允许移除旧清单或内容对象。
- 任何本地恢复路径都必须先验证真实路径属于允许的备份根目录。
- Git checkout 后的工作区复原必须放在 `finally` 路径。
- 备份/恢复保持进程内互斥；未来增加跨进程锁时应覆盖异常退出。
- 修改备份恢复时必须覆盖旧链基准缺失/环/越界、Codex-only、删除 tombstone、对象损坏、路径穿越、full 回滚和块去重。
