...
这种机制类似精简的现代快照备份仓库：读取当前仓库状态、复用目标仓库已有对象、提交一份新清单。`size + mtime + ctime + 文件标识` 全部未变时直接复用上一份已经哈希的文件记录；任一项变化才重新读取并按内容哈希，因此不是只依赖 `mtime`。活跃大对话追加少量内容时通常只增加尾部块，而不是再次保存整个文件。固定分块对“中间插入”不如内容定义分块高效，但会话 JSONL 的主要写入模式是末尾追加。
